...
def editar_slice(slice_api, auth_manager):
    ...
    try:
        # Leer slices locales del usuario
        ...
        if user_role == 'admin':
            # Admin puede editar todos los slices
        ...
        try:
            ...
            if 0 <= idx < len(slices_usuario):
                ...
                cambios = False
                if edit_choice == '1':
                    agregar_vms_a_slice(slice_seleccionado, data, VMS_JSON, BASE_YAML)
                    cambios = True
                elif edit_choice == '2':
                    modificar_topologia_slice(slice_seleccionado, data, BASE_YAML)
                    cambios = True
                elif edit_choice == '3':
                    eliminar_vms_de_slice(slice_seleccionado, data, VMS_JSON, BASE_YAML)
                    cambios = True
                elif edit_choice == '0':
                ...
                if cambios:
                    show_success("Cambios guardados en base_de_datos.yaml y vms.json")
            else:
                print(f"\n{Colors.RED}  ❌ Opción inválida{Colors.ENDC}")
...
def agregar_vms_a_slice(slice_seleccionado, data, VMS_JSON, BASE_YAML):
    ...
    try:
        # Pedir cantidad de VMs a agregar
        ...
        # Actualizar slice en base_de_datos.yaml
        for s in data['slices']:
            if s.get('id') == slice_seleccionado.get('id'):
                s['vms'] = vms_actuales
                break
